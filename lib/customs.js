...
module.exports = function (log, error) {
  ...
  Customs.prototype.checkAuthenticated = function (request, email, action) {
    log.trace({ op: 'customs.checkAuthenticated', email: email, action: action })
    return this.pool.post(
      '/checkAuthenticated',
      {
        // NOTE: todo add our params
      }
    )
      // NOTE: add auth-servers here, just like the /check endpoint
      // such as `requestBlocked`
  }
